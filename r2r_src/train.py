...
from utils import read_vocab,write_vocab,build_vocab,Tokenizer,padding_idx,timeSince, read_img_features, convert_dict, base_vocab
import utils
...
def train_speaker(train_env, tok, n_iters, log_every=500, val_envs={}):
    ...
    for idx in range(0, n_iters, log_every):
        ...
        for env_name, (env, evaluator) in val_envs.items():
            ...
            e[idx+interval][env_name]={"Bleu":tuple(precisions),"word_accu": word_accu, " sent_accu":  sent_accu,  "bleu_score":bleu_score}
            
            # Screen print out
            print("Bleu 1: %0.4f Bleu 2: %0.4f, Bleu 3 :%0.4f,  Bleu 4: %0.4f" % tuple(precisions))
            #print(env_name, env, evaluator)
            #with open("tasks/R2R/data/speaker_data.json", "a+") as fi:
              #  fi.write(json.dumps(e, indent=3))
            #with open("tasks/R2R/data/loss_speaker_avg.json", "a+") as fi:
...
def train(train_env, tok, n_iters, log_every=100, val_envs={}, aug_env=None):
    ...
    with open("output/train_score/train.csv", 'w') as f:
        f.write("idx,total,length,critic_loss,entropy,predict_loss")
        f.write("\n")
    ...
    for idx in range(start_iter, start_iter+n_iters, log_every):
        ...
        #print("ITER IN TRAIN and env", iter, train_env)
        #print("start_iter", start_iter,"n_iters", n_iters, "idx", idx, "interval", interval)
 ...
 # Train for log_every interval
        ...
        #print("listner logs", listner.logs)
 ...
 #print("total_actions", total)
        ...
        with open("output/train_score/train.csv", 'a') as f:
            f.write('%d, %d, %d, %d, %d, %d' % (idx,total,length,critic_loss,entropy,predict_loss))
            f.write("\n")
...
def setup():
    ...
    if not os.path.exists(TRAIN_VOCAB):
        ...
        if not os.path.exists("tasks/R2R/data/%s_glove.json" % s):
            index_to_embed[1] = ["0"] * 300
            new_e = defaultdict(lambda: index_to_embed[1])

            print("...getting glove embeddings")
            f = open("tasks/R2R/data/glove.42B.300d.txt", 'r', encoding="utf-8")
            glove_embed = convert_dict(f)
            new_e.update(glove_embed)
            glove_embed = new_e
            for i, w in enumerate(vocab):
                index_to_embed[i] = glove_embed[w]

            index_to_embed[0] = [str((len(vocab) - 1 / len(vocab)))] * 300
            index_to_embed[2] = ["1"] * 300
            index_to_embed[3] = ["-1"] * 300

            with open("tasks/R2R/data/%s_glove.json" % s, "w") as fi:
                fi.write(json.dumps(index_to_embed))
                print("Embeddings saved in tasks/R2R/data/%s_glove.json" % s)
